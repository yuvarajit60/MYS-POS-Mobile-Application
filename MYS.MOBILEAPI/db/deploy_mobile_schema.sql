/*
  ...
  Also folds in 029_cancel_entry_and_ledger_updates.sql (see that file's
  header): CANCELREMARKS added to SALESORDER/TRIPENTRY/DELIVERY_DETAILS/
  PAYMENT_DETAILS for the new common Cancel Entry feature, and
  SP_MOBILE_GET_CUSTOMER_LEDGER now excludes cancelled Delivery/Payment rows.
...
-- SITE is site master data only (no City/Customer) — see 026_site_customer_mapping_split.sql.
-- City is derived via AREAID -> AREA.CityId -> CITY.CITYID; Customer lives in SITE_MAPPING.
...
BEGIN
    ...
    (
        ...
        AREAID              INT          NOT NULL CONSTRAINT DF_SITE_AREAID DEFAULT 0,
...
-- STATUS = 1 marks the currently active Site-to-Customer mapping for a
-- SITEID (at most one active row per site); remapping/unmapping closes the
-- old active row instead of overwriting it, so history is kept.
...
------------------------------------------------------------
-- Split dbo.SITE's old AREANAME/CITYID/CUSTOMERID columns out into
-- dbo.SITE_MAPPING (see 026_site_customer_mapping_split.sql for the full
-- rationale). Backfills one active SITE_MAPPING row per SITE row that
-- still has a CUSTOMERID before dropping it, so already-live databases
-- (db_ams_erp, DB_AMS_ERP_SMS) keep every existing mapping.
------------------------------------------------------------
...
DECLARE @SiteSplitConstraintName SYSNAME;
...
BEGIN
    SELECT TOP 1 @SiteSplitConstraintName = dc.name
    FROM sys.default_constraints dc
    JOIN sys.columns c ON c.object_id = dc.parent_object_id AND c.column_id = dc.parent_column_id
    WHERE dc.parent_object_id = OBJECT_ID('dbo.SITE') AND c.name IN ('AREANAME', 'CITYID', 'CUSTOMERID');

    EXEC('ALTER TABLE dbo.SITE DROP CONSTRAINT [' + @SiteSplitConstraintName + ']');
END
GO
...
------------------------------------------------------------
-- Cancel Entry (folded in from 029_cancel_entry_and_ledger_updates.sql —
-- see that file's header). CANCEL/CANCELUSERID/CANCELDATETIME|CANCELDATE
-- already exist on all four tables; only CANCELREMARKS is new here.
------------------------------------------------------------
